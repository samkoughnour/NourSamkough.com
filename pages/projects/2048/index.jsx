...
function App() {
  ...
  const detectCollision = (direction, placeholderBoard) => {
    if (direction === 37) {
      ...
      placeholderBoard = placeholderBoard[0].map((_, colIndex) =>
        placeholderBoard.map((row) => row[colIndex]),
      )
      for (let i = 0; i < placeholderBoard.length; i++) {
        let last = placeholderBoard[i][0]
        for (let j = 1; j < placeholderBoard[0].length; j++) {
  ...
  const checkStill = (placeholderBoard, keyCode) => {
    ...
    if (
      JSON.stringify(placeholderBoard) ==
        JSON.stringify(shiftBoard(keyCode, placeholderBoard)) &&
      zeroCount !== placeholderBoard.length * placeholderBoard[0].length
    ) {
      return true
    }
  }
